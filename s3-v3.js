// TypingMind Cloud Sync & Backup v2.2.0 (Complete Performance Refactor)
// This version uses an Inline Web Worker to offload all heavy cryptographic,
// compression, and large JSON processing tasks, preventing browser crashes
// with large data sets. All logic is contained within this single file to
// comply with TypingMind's extension system.
// It also ensures consistent and efficient IndexedDB usage.

const EXTENSION_VERSION = "2.2.0";

// =================================================================================
// == INLINE WEB WORKER CODE (defined as a string)
//    This code runs in a separate background thread and does not block the UI.
// =================================================================================
const workerCode = `
    // Import JSZip library for compression/decompression
    self.importScripts('https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js');

    // Helper function for key derivation (copied from main script)
    ...
    self.onmessage = async (event) => {
        ...
        try {
            if (type === 'encrypt') {
                ...
                // Heavy JSON.stringify is now off the main thread!
                const jsonString = JSON.stringify(data, (key, value) => (value === undefined ? null : value));
                ...
                self.postMessage({ success: true, result: combinedData.buffer }, [combinedData.buffer]);
                ...
                const encryptedData = data.slice(markerLength + 12);
                ...
                // Heavy JSON.parse is now off the main thread!
                const parsedData = JSON.parse(decryptedText); 
                self.postMessage({ success: true, result: parsedData });

            } else if (type === 'hash') {
                const { content, hashType } = payload;
                let str;
                if (hashType === "chat" && content.id) {
                    ...
                    const stableChat = {
                        ...
                        messages: messagesToProcess.map(msg => {
                            ...
                            const stringifyStable = (obj) => JSON.stringify(obj, Object.keys(obj || {}).sort());
                ...
                self.postMessage({ success: true, result: hashHex });
                ...
                const compressedContent = await zip.generateAsync({ type: "arraybuffer" });
                self.postMessage({ success: true, result: compressedContent }, [compressedContent]);

            } else if (type === 'unzip') {
                const zip = await self.JSZip.loadAsync(payload.data);
                ...
                self.postMessage({ success: true, result: backupContent.buffer }, [backupContent.buffer]);
            }
...
// Helper function to create a worker from the code string above.
function createInlineWorker(code) {
    try {
        const blob = new Blob([code], { type: 'application/javascript' });
        const url = URL.createObjectURL(blob);
        const worker = new Worker(url);
        URL.revokeObjectURL(url); // Clean up the blob URL immediately after creating worker
        return worker;
    } catch (e) {
...
// Generic helper to call the worker and get a response via a Promise.
function callWorker(type, payload) {
    return new Promise((resolve, reject) => {
        const worker = createInlineWorker(workerCode);
        if (!worker) {
            return reject(new Error("Could not create worker. Your browser might not support them or be in a restricted context."));
        }
        ...
        worker.onmessage = (event) => {
            ...
            worker.terminate(); // Terminate worker after use to free resources
        };
        ...
        // Use transferable objects for ArrayBuffers to improve performance
        const transferList = [];
        if (payload.data instanceof ArrayBuffer) {
            transferList.push(payload.data);
        } else if (payload.data instanceof Uint8Array) {
            transferList.push(payload.data.buffer);
        }

        worker.postMessage({ type, payload }, transferList);
    });
}

// =================================================================================
// == REFACTORED HEAVY-LIFTING FUNCTIONS (now use the Web Worker)
// =================================================================================
...
async function encryptData(data) {
    ...
    try {
        logToConsole("info", "Offloading encryption to worker...");
        // Data is passed directly, JSON.stringify happens in worker
        const encryptedBuffer = await callWorker('encrypt', { data, encryptionKey });
        return new Uint8Array(encryptedBuffer);
    } catch (error) {
        logToConsole("error", "Encryption failed in worker:", error);
        throw error;
    }
}
...
async function decryptData(data) {
    const marker = "ENCRYPTED:";
    // Data is a Uint8Array, check marker on main thread first
    const dataString = new TextDecoder().decode(data.slice(0, marker.length));
    
    if (dataString !== marker) {
        logToConsole("info", "Data is not encrypted, parsing on main thread.");
        // If not encrypted, parse JSON directly (still blocking for very large unencrypted files)
    ...
    if (!encryptionKey) {
        ...
        // showCustomAlert is not defined in this scope, assuming it's a global or provided by TypingMind
    ...
    try {
        logToConsole("info", "Offloading decryption to worker...");
        // Pass data.buffer to allow transfer, worker will return parsed JSON object
        const decryptedData = await callWorker('decrypt', { data: data.buffer, encryptionKey });
        logToConsole("success", "Decryption successful in worker.");
        return decryptedData; // Worker now returns parsed JSON object
    } catch (error) {
        logToConsole("error", "Decryption failed in worker:", error);
...
async function generateHash(content, type = "generic") {
    try {
        // Pass content and type to worker, worker handles stringify and hash
        const hash = await callWorker('hash', { content, hashType: type });
        return hash;
    } catch (error) {
        logToConsole("error", "Hashing failed in worker:", error);
        throw error;
    }
}
...
async function createDailyBackup(key, data) {
  ...
  try {
    logToConsole("info", "Encrypting backup data in worker...");
    // Encrypts the data (which is a JS object/string from exportBackupData)
    const encryptedData = await encryptData(data); // Returns Uint8Array
    
    logToConsole("info", "Zipping backup data in worker...");
    // Zips the encrypted data (Uint8Array)
    const compressedBuffer = await callWorker('zip', { data: encryptedData.buffer, key: key }); // Pass buffer for transfer

    if (compressedBuffer.byteLength < 100) {
      throw new Error("Daily backup file is too small or empty. Upload cancelled.");
    }
    
    const content = new Uint8Array(compressedBuffer);
    const uploadMetadata = {
      version: EXTENSION_VERSION,
      timestamp: String(Date.now()),
      type: "daily",
      originalSize: String(encryptedData.length), // Size of encrypted (but unzipped) data
      compressedSize: String(content.length),
      encrypted: "true",
    };
...
async function createSnapshot(name) {
  ...
  try {
    const data = await exportBackupData(); // Gets all data as a JS object
    const now = new Date();
    ...
    logToConsole("info", "Encrypting snapshot data in worker...");
    const encryptedData = await encryptData(data); // Returns Uint8Array
    
    logToConsole("info", "Zipping snapshot data in worker...");
    const compressedBuffer = await callWorker('zip', { data: encryptedData.buffer, key: key }); // Pass buffer for transfer
    ...
    const content = new Uint8Array(compressedBuffer);
    const uploadMetadata = {
        version: EXTENSION_VERSION, timestamp: String(Date.now()), type: "snapshot",
        originalSize: String(encryptedData.length), // Size of encrypted (but unzipped) data
        compressedSize: String(content.length),
        encrypted: "true",
    };
...
async function restoreFromBackup(key) {
  ...
  try {
    ...
    const backup = await downloadFromS3(key); // Returns { data: Uint8Array, metadata: {} }
    if (!backup || !backup.data) {
      throw new Error("Backup not found or empty");
    }
    
    let backupContentBuffer; // Will hold ArrayBuffer after unzip
    if (key.endsWith(".zip")) {
      logToConsole("info", "Unzipping backup in worker...");
      // Pass backup.data.buffer for transfer, worker returns ArrayBuffer
      backupContentBuffer = await callWorker('unzip', { data: backup.data.buffer });
    } else {
      backupContentBuffer = backup.data.buffer; // If not zipped, use original buffer
    }
    
    logToConsole("info", "Decrypting and parsing backup content in worker...");
    // Pass buffer for transfer, worker returns parsed JSON object
    const parsedContent = await decryptData(new Uint8Array(backupContentBuffer)); 
    ...
    if (!parsedContent.localStorage && !parsedContent.indexedDB) {
      throw new Error("Invalid backup format: Missing both localStorage and indexedDB sections");
    }
...
// =================================================================================
// == ORIGINAL SCRIPT - Remaining Functions (with minor adjustments for consistency)
// =================================================================================

const EXCLUDED_SETTINGS = [
  "aws-bucket", "aws-access-key", "aws-secret-key", "aws-region", "aws-endpoint",
  "encryption-key", "chat-sync-metadata", "sync-mode", "last-cloud-sync",
  "TM_useDraftContent", "last-daily-backup", "TM_useLastVerifiedToken",
  "TM_useStateUpdateHistory", "TM_useGlobalChatLoading", "TM_crossTabLastSynced",
  "TM_useLastOpenedChatID", "INSTANCE_ID",
];

function getUserDefinedExclusions() {
  const exclusions = localStorage.getItem("sync-exclusions");
  return exclusions ? exclusions.split(",").map((item) => item.trim()).filter((item) => item) : [];
}
...
function shouldExcludeSetting(key) {
  ...
  if (isExcluded && userExclusions.includes(key)) {
    logToConsole("debug", `Setting excluded by user-defined exclusions: ${key}`);
  }
  return isExcluded;
}

let config = {
  syncMode: "disabled", syncInterval: 15, bucketName: "", region: "",
  accessKey: "", secretKey: "", endpoint: "", encryptionKey: "",
};
...
let localMetadata = {
  chats: {},
  settings: { items: {}, lastModified: 0, syncedAt: 0 },
  lastSyncTime: 0,
};
...
let operationState = {
  isImporting: false, isExporting: false, isPendingSync: false,
  operationQueue: [], isProcessingQueue: false, lastSyncStatus: null,
  isCheckingChanges: false, lastError: null, operationStartTime: null,
  queueProcessingPromise: null, completedOperations: new Set(), operationTimeouts: new Map(),
};

let backupState = {
  isBackupInProgress: false, lastDailyBackup: null, lastManualSnapshot: null,
  backupInterval: null, isBackupIntervalRunning: false,
};
...
let activeIntervals = { sync: null, backup: null, changeCheck: null };

function clearAllIntervals() {
  if (activeIntervals.sync) { clearInterval(activeIntervals.sync); activeIntervals.sync = null; }
  if (activeIntervals.backup) { clearInterval(activeIntervals.backup); activeIntervals.backup = null; }
  if (activeIntervals.changeCheck) { clearInterval(activeIntervals.changeCheck); activeIntervals.changeCheck = null; }
}

const LOG_ICONS = {
  info: "ℹ️", success: "✅", warning: "⚠️", error: "❌", start: "🔄", end: "🏁", upload: "⬆️",
  download: "⬇️", cleanup: "🧹", snapshot: "📸", encrypt: "🔐", decrypt: "🔓", progress: "📊",
  time: "⏰", wait: "⏳", pause: "⏸️", resume: "▶️", visibility: "👁️", active: "📱",
  calendar: "📅", tag: "🏷️", stop: "🛑", skip: "⏩",
};
...
function logToConsole(type, message, data = null) {
  ...
  if (/Mobi|Android/i.test(navigator.userAgent)) {
      ...
      if (logsContent) {
          ...
          const isSearchActive = searchInput && !searchInput.classList.contains("hidden");
          ...
          if (isSearchActive) {
              ...
              if (query && logMessage.toLowerCase().includes(query.toLowerCase())) {
                  const isReversed = container.getAttribute("data-log-reversed") === "true";
                  if (isReversed) {
                      logsContent.insertBefore(logEntry.cloneNode(true), logsContent.firstChild);
                  } else {
...
function createMobileLogContainer() {
  ...
  minimizedTag.className = "fixed bottom-0 right-0 bg-black text-white px-3 py-1 m-2 rounded cursor-pointer z-[9999] hidden";
  ...
  function endDragTag() {
    ...
    if (isDraggingTag) {
      ...
      const rect = minimizedTag.getBoundingClientRect();
      localStorage.setItem("mobile-log-tag-position", JSON.stringify({ x: rect.left, y: rect.top }));
    }
    stopLongPress();
  }
  ...
  searchInput.className = "hidden bg-gray-700 text-white px-2 py-1 rounded text-sm flex-1 focus:outline-none focus:ring-1 focus:ring-blue-500";
  ...
  function restoreOriginalLogs() {
    ...
    if (logsContainer && container.originalLogEntries.length > 0) {
      ...
      container.originalLogEntries.forEach((entry) => logsContainer.appendChild(entry));
    }
  }
  ...
  reverseBtn.addEventListener("click", (e) => {
    ...
    if (logsContainer && logsContainer.children.length > 0) {
      ...
      reverseBtn.title = isReversed ? "Latest first (click to reverse)" : "Latest last (click to reverse)";
    }
  });
...
function initializeLoggingState() {
  ...
  if (logParam === "true") {
    ...
    logToConsole("info", `TypingMind Cloud Sync & Backup v${EXTENSION_VERSION} initializing...`);
  }
}
...
async function performFullInitialization() {
  try {
    ...
    monitorIndexedDBForDeletions(); // This calls getAllChatsFromIndexedDB internally
    startPeriodicChangeCheck(); // This calls getAllChatsFromIndexedDB internally
    setupVisibilityChangeHandler();
    try {
      await cleanupMetadataVersions();
      logToConsole("success", "Metadata cleanup completed during initialization");
    } catch (cleanupError) {
      logToConsole("warning", "Non-critical: Metadata cleanup failed during initialization", cleanupError);
    }
  ...
  try {
    ...
    try {
      ...
      if (duplicatesFound) {
        ...
        logToConsole("success", "IndexedDB duplicate cleanup completed during extension initialization");
      } else {
        logToConsole("info", "No duplicates found - skipping cleanup");
      }
    } catch (cleanupError) {
      logToConsole("warning", "Non-critical: IndexedDB duplicate cleanup failed during extension initialization", cleanupError);
    }
    let hashesRecalculated = false;
    const allLocalChatsForHash = await getAllChatsFromIndexedDB();
    const localChatsMapForHash = new Map(allLocalChatsForHash.map((chat) => [chat.id.replace(/^CHAT_/, ""), chat]));
      ...
      for (const chatId in localMetadata.chats) {
        ...
        if (chatData && !localMetadata.chats[chatId].deleted) {
          try {
            ...
            logToConsole("error", `Error generating hash for chat ${cleanChatId} during init recalc`, hashError);
          }
        } else if (!chatData && !localMetadata.chats[chatId].deleted) {
          logToConsole("warning", `Chat ${cleanChatId} found in metadata but not in IndexedDB during hash recalc.`);
        }
      }
    }
    if (initialMetadataSaveNeeded || settingsMetadataSaveNeeded || hashesRecalculated) {
      await saveLocalMetadata();
    }
    await setupLocalStorageChangeListener();
    startSyncInterval();
    ...
    if (config.syncMode === "sync") {
      ...
      const cloudIsEmptyOrNew = !cloudMetadata || !cloudMetadata.chats || Object.keys(cloudMetadata.chats).length === 0 || cloudMetadata.lastSyncTime === 0;
      const localHasData = localMetadata && localMetadata.chats && Object.keys(localMetadata.chats).length > 0;

      if (cloudIsEmptyOrNew && localHasData) {
        logToConsole("info", "Cloud is empty/new but local data exists. Performing initial sync/upload.");
        await queueOperation("initial-sync-upload", performInitialSync, [], 300000);
      } else if (!cloudIsEmptyOrNew) {
        logToConsole("info", "Cloud data found. Performing standard startup sync check.");
        if (document.visibilityState === "visible") {
          queueOperation("startup-sync-check", syncFromCloud, [], 300000);
        }
      } else {
        logToConsole("info", "Both cloud and local seem empty or new. No initial sync needed immediately.");
      }
    }
    if (config.syncMode !== "disabled") {
      queueOperation("daily-backup-check", checkAndPerformDailyBackup, [], 300000);
...
async function loadJSZip() {
  // This function is technically not needed on the main thread anymore
  // as JSZip is imported directly in the worker. Keeping it won't hurt.
  ...
  try {
    ...
    if (storedMetadata) {
      try {
        ...
        const formatLogTimestamp = (ts) => ts ? new Date(ts).toLocaleString() : ts === 0 ? "0 (Epoch)" : ts;
        ...
        if (localMetadata.settings?.items) {
          ...
          if (sampleKeys.length > 0) {
            settingsHashSamples = sampleKeys.reduce((acc, key) => {
              acc[key] = localMetadata.settings.items[key]?.hash ? `${localMetadata.settings.items[key].hash.substring(0, 8)}...` : "none";
              return acc;
            }, {});
          }
        }
        logToConsole("debug", "Parsed localMetadata:", {
          ...
          chatCount: localMetadata.chats ? Object.keys(localMetadata.chats).length : 0,
          firstChatSyncedAt:
            localMetadata.chats && Object.keys(localMetadata.chats).length > 0
              ? formatLogTimestamp(localMetadata.chats[Object.keys(localMetadata.chats)[0]]?.syncedAt)
              : undefined,
          hasSettings: !!localMetadata.settings,
          settingsCount: localMetadata.settings?.items ? Object.keys(localMetadata.settings.items).length : 0,
          settingsSyncedAt: formatLogTimestamp(localMetadata.settings?.syncedAt),
          settingsSamples: settingsHashSamples,
        });
      } catch (parseError) {
        logToConsole("error", "Failed to parse stored metadata, initializing from scratch", parseError);
        metadataInitialized = await initializeMetadataFromExistingData();
      }
    } else {
      logToConsole("info", "No stored metadata found, initializing from existing data.");
      metadataInitialized = await initializeMetadataFromExistingData();
    }
  } catch (error) {
    logToConsole("error", "Failed to load local metadata:", error);
    try {
      logToConsole("warning", "Attempting to recover by initializing fresh metadata.");
      metadataInitialized = await initializeMetadataFromExistingData();
      logToConsole("success", "Successfully initialized fresh metadata after load error.");
    } catch (initError) {
      logToConsole("error", "Failed to initialize fresh metadata after load error:", initError);
      throw error;
    }
  }
          ...
          if (isEstablishedChat) {
            ...
            if (missingCount >= REQUIRED_MISSING_DETECTIONS) {
              ...
              if (config.syncMode === "sync" || config.syncMode === "backup") {
                logToConsole("cleanup", `Queueing deletion from cloud for chat ${chatId}`);
    ...
    const objectsWithMetadata = await Promise.all(
      objects.map(async (obj) => {
        try {
          ...
          logToConsole("error", `Failed to get metadata for ${obj.Key}:`, error);
          return {
            ...obj,
            key: obj.Key,
            metadata: {},
  ...
  try {
    ...
    if (data.byteLength > 5 * 1024 * 1024) {
      ...
      for (let i = 0; i < data.byteLength; i += partSize) {
        ...
        parts.push(part);
        const progress = Math.min(100, Math.round((end / data.byteLength) * 100));
      }
...
async function downloadFromS3(key) {
  ...
  try {
    ...
    if (key === "metadata.json") {
      return {
        data: new Uint8Array(response.Body), // Ensure Uint8Array for consistency
...
async function uploadPart(key, uploadId, partNumber, data) {
  ...
  while (!uploadSuccess && retryCount <= maxRetries) {
    try {
      ...
      if (retryCount > 0) {
        logToConsole("info", `Retrying upload part ${partNumber} (attempt ${retryCount + 1}/${maxRetries + 1})`);
      }
      ...
      lastError = error;
      retryCount++;
      logToConsole("error", `Error uploading part ${partNumber} (attempt ${retryCount}/${maxRetries + 1}):`, error);
      if (retryCount > maxRetries) {
        ...
        throw new Error(`Failed to upload part ${partNumber} after ${maxRetries + 1} attempts: ${lastError.message}`);
      }
      const delay = Math.min(baseDelay * Math.pow(2, retryCount - 1) + Math.random() * 1000, 30000);
...
async function cleanupIncompleteMultipartUploads() {
  const s3 = initializeS3Client();
  try {
    const multipartUploads = await s3.listMultipartUploads({ Bucket: config.bucketName }).promise();
    if (multipartUploads.Uploads && multipartUploads.Uploads.length > 0) {
      ...
      for (const upload of multipartUploads.Uploads) {
        ...
        if (uploadAge > fiveMinutes) {
          try {
            ...
            logToConsole("error", `Failed to abort upload for ${upload.Key}:`, error);
          }
        } else {
      ...
      // logToConsole("debug", "No incomplete multipart uploads found.");
    }
  } catch (error) {
...
// NOTE: safeStringify is no longer needed on the main thread for crypto operations
// as JSON.stringify is handled in the worker. This function is kept for completeness
// but may not be called.
async function safeStringify(data) {
  try {
    return JSON.stringify(data, (key, value) => value === undefined ? null : value);
  } catch (error) {
    logToConsole("warning", "Safe stringify failed, falling back to regular JSON.stringify", error);
    return JSON.stringify(data, (key, value) => value === undefined ? null : value);
  }
}
...
async function performDailyBackup() {
  ...
  try {
    ...
    const data = await exportBackupData(); // Gets raw JS object
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        allKeys.forEach((key, i) => {
            exportData.indexedDB[key] = allEntries[i];
        });
        ...
        logToConsole("error", "Error collecting IndexedDB data for export:", error);
        reject(error);
    }
  });
}
...
async function cleanupOldBackups(type) {
  ...
  try {
    ...
    switch (type) {
      case "daily":
        prefix = "typingmind-backup-"; // Assuming this prefix
        keepCount = 7; // Example: keep last 7 daily backups
        break;
      default:
        return;
    }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (data.localStorage) {
        Object.entries(data.localStorage).forEach(([key, settingData]) => {
            if (!preserveKeys.includes(key)) {
                try {
                    const value = settingData; // Direct value from parsed JSON
    ...
    if (data.indexedDB) {
        try {
            ...
            logToConsole("error", "IndexedDB restore failed:", error);
            reject(error);
        }
    } else {
        logToConsole("success", `Settings restore completed (no IndexedDB data)`, {
            totalRestored: settingsRestored,
            timestamp: new Date().toISOString(),
        });
        resolve();
    ...
    try {
      ...
      while (operationState.operationQueue.length > 0) {
        ...
        if (nextOpIndex === -1) {
          ...
          logToConsole(
            "error",
            `Dependency cycle or missing dependencies detected. Missing: ${JSON.stringify(missingDeps)}`,
            {
          ...
          operationState.operationQueue = operationState.operationQueue.filter(
            (op) => {
              ...
              if (opMissingDeps.length > 0) {
                logToConsole(
                  "warning",
                  `Removing operation '${op.name}' due to missing dependencies: ${JSON.stringify(opMissingDeps)}`
                );
                return false;
              }
              return true;
...
async function detectCloudChanges(cloudMetadata) {
  ...
  if (cloudMetadata.settings) {
    ...
    if (
      ...
      logToConsole(
        "debug",
        `✅ Cloud settings change detected: Cloud lastModified ${new Date(cloudLastModified).toISOString()} > Local syncedAt ${new Date(localSyncedAt).toISOString()}`
      );
      return true;
    } else {
      logToConsole(
        "debug",
        `❌ No settings changes: Cloud lastModified ${new Date(cloudLastModified).toISOString()} <= Local syncedAt ${new Date(localSyncedAt).toISOString()}`
      );
    }
  }
    ...
    if (
      ...
      if (settingsWereApplied) {
        ...
        if (cloudMetadata.settings && cloudMetadata.settings.lastModified) {
          localMetadata.settings.lastModified = cloudMetadata.settings.lastModified;
        } else {
          localMetadata.settings.lastModified = syncTimestamp;
        }
        metadataNeedsSaving = true;
        hasChanges = true;
        logToConsole("success", `Settings sync completed. Local settings marked synced at ${new Date(syncTimestamp).toISOString()}`);
      } else {
        if (cloudMetadata.settings && cloudMetadata.settings.lastModified > localMetadata.settings.syncedAt) {
          localMetadata.settings.syncedAt = cloudMetadata.settings.lastModified;
          localMetadata.settings.lastModified = cloudMetadata.settings.lastModified;
          metadataNeedsSaving = true;
          logToConsole("info", "Updated settings syncedAt timestamp even though no content changed.");
        }
      }
    }
...
async function updateChatMetadata(
  ...
  if (!chat && !isDeleted) {
    logToConsole("error", "Chat object not provided to updateChatMetadata for non-deletion", chatId);
    return false;
  }
  if (!localMetadata.chats[chatId]) {
    localMetadata.chats[chatId] = {
  ...
  if (chat) {
    ...
    if (isModified && !syncTimestamp) {
      ...
      logToConsole("info", `Queueing upload for locally modified chat ${chatId}`);
...
function setupVisibilityChangeHandler() {
  document.addEventListener("visibilitychange", () => {
    ...
    logToConsole("visibility", `Page visibility changed: ${isVisible ? "visible" : "hidden"}`);
    if (isVisible) {
      if (config.syncMode === "disabled") {
        logToConsole("info", "Cloud operations disabled - skipping visibility change handling");
        return;
      }
      if (config.syncMode === "sync") {
...
function insertSyncButton() {
  ...
  button.innerHTML = `
    <span class="text-white/70 hover:bg-white/20 self-stretch h-12 md:h-[50px] px-0.5 py-1.5 rounded-xl flex-col justify-start items-center gap-1.5 flex transition-colors">
      ...
      <span class="font-normal self-stretch text-center text-xs leading-4 md:leading-none ${config.syncMode === "disabled" ? "text-gray-400 dark:text-gray-500" : ""}">${config.syncMode === "sync" ? "Sync" : "Backup"}</span>
    </span>
  `;
          ...
          <div class="space-y-2">
            <div class="flex items-center space-x-4 mb-4">
              <label class="inline-flex items-center">
                <input type="radio" name="sync-mode" value="sync" class="form-radio text-blue-600" ${config.syncMode === "sync" ? "checked" : ""}>
              ...
              <label class="inline-flex items-center">
                <input type="radio" name="sync-mode" value="backup" class="form-radio text-blue-600" ${config.syncMode === "backup" ? "checked" : ""}>
              ...
              <label class="inline-flex items-center">
                <input type="radio" name="sync-mode" value="disabled" class="form-radio text-blue-600" ${config.syncMode === "disabled" ? "checked" : ""}>
            ...
            <div class="flex space-x-4">
              <div class="w-2/3">
                ...
                <input id="aws-bucket" name="aws-bucket" type="text" value="${config.bucketName || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
              </div>
              <div class="w-1/3">
                ...
                <input id="aws-region" name="aws-region" type="text" value="${config.region || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
              </div>
            </div>
            <div>
              ...
              <input id="aws-access-key" name="aws-access-key" type="password" value="${config.accessKey || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
            </div>
            <div>
              ...
              <input id="aws-secret-key" name="aws-secret-key" type="password" value="${config.secretKey || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
            </div>
            <div>
              ...
              <input id="aws-endpoint" name="aws-endpoint" type="text" value="${config.endpoint || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off">
            </div>
            <div class="flex space-x-4">
              <div class="w-1/2">
                ...
                <input id="sync-interval" name="sync-interval" type="number" min="15" value="${config.syncInterval}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
              </div>
              <div class="w-1/2">
                ...
                <input id="encryption-key" name="encryption-key" type="password" value="${config.encryptionKey || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" autocomplete="off" required>
              </div>
            </div>
            <div>
              ...
              <input id="sync-exclusions" name="sync-exclusions" type="text" value="${localStorage.getItem("sync-exclusions") || ""}" class="z-1 w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700" placeholder="e.g., my-setting, another-setting" autocomplete="off">
            </div>
          </div>
        </div>
      ...
      if (key === "settings.json") {
        if (
          ...
          try {
            ...
            const decryptedContent = await decryptData(backup.data); // This now returns parsed JSON
            const settingsData = decryptedContent; // Use directly
            const cloudMetadata = await downloadCloudMetadata();
            const preserveKeys = [
              "aws-bucket", "aws-access-key", "aws-secret-key", "aws-region", "aws-endpoint",
              "encryption-key", "chat-sync-metadata",
            ];
            let settingsRestored = 0;
            for (const [key, settingValue] of Object.entries(settingsData)) {
              if (!preserveKeys.includes(key)) {
                try {
                  const value = settingValue.data; // Access .data property
                  if (settingValue.source === "indexeddb") {
                    await setIndexedDBKey(key, value);
                    logToConsole("info", `Restored setting to IndexedDB: ${key}`);
                  } else {
                    localStorage.setItem(key, value);
                    logToConsole("info", `Restored setting to localStorage: ${key}`);
                  ...
                  logToConsole("error", `Error restoring setting ${key}:`, error);
                }
              }
            }
            if (cloudMetadata.settings) {
            ...
            alert(`Settings restored successfully! (${settingsRestored} settings restored)`);
          } catch (error) {
...
async function handleZipDownload(backup, key) {
  try {
    // backup.data is Uint8Array
    const unzippedBuffer = await callWorker('unzip', { data: backup.data.buffer });
    const decryptedContent = await decryptData(new Uint8Array(unzippedBuffer)); // decryptData now returns parsed JSON
    const blob = new Blob([JSON.stringify(decryptedContent, null, 2)], { type: "application/json" });
...
async function handleRegularFileDownload(backup, key) {
  try {
    const decryptedContent = await decryptData(backup.data); // decryptData returns parsed JSON
    if (key.endsWith(".json")) {
      const blob = new Blob([JSON.stringify(decryptedContent, null, 2)], { type: "application/json" });
      downloadFile(key, blob);
    } else {
      // If it's not JSON, it might be raw decrypted text or binary if encryption failed.
      // This path might need more specific handling based on expected file types.
      downloadFile(key, JSON.stringify(decryptedContent)); // Fallback to stringifying if it's an object
    }
  } catch (error) {
    logToConsole("error", "Processing failed, downloading raw data:", error);
    downloadFile(key, backup.data); // fallback to downloading raw encrypted data
  }
}
...
function downloadFile(filename, data) {
  ...
  if (data instanceof Blob) {
    ...
    try {
      ...
      logToConsole("error", "Failed to stringify object for download:", error);
...
// NOTE: getIndexedDBValue is now an alias for getIndexedDBKey which uses the persistent DB
// This function is still used internally but the underlying implementation is changed.
async function getIndexedDBValue(key) {
  return await getIndexedDBKey(key);
}
...
// NOTE: generateContentHash now uses the worker for hashing.
async function generateContentHash(content) {
    const str = typeof content === "string" ? content : JSON.stringify(content);
    const msgBuffer = new TextEncoder().encode(str);
    try {
        const hash = await callWorker('hash', { content: msgBuffer.buffer, hashType: 'generic' });
        return hash;
    } catch (error) {
        logToConsole("error", "Content hashing failed in worker:", error);
        throw error;
    }
}
...
async function checkIndexedDBChanges() {
  ...
  try {
    ...
    if (changedKeys.size > 0) {
      logToConsole(
        "info",
        `Detected changes in ${changedKeys.size} IndexedDB items: ${Array.from(changedKeys).join(", ")}`
      );
...
async function cleanupMetadataVersions() {
  try {
    ...
    const versioningStatus = await s3.getBucketVersioning({ Bucket: config.bucketName }).promise();
    const isVersioningEnabled = versioningStatus.Status === "Enabled";
    if (!isVersioningEnabled) {
      logToConsole("info", "Bucket versioning is not enabled, skipping version cleanup");
      return;
    }
    ...
    if (versions.Versions) {
      allVersions.push(
        ...versions.Versions.filter((v) => !v.IsLatest && v.Key === "metadata.json")
      );
    }
    ...
    if (versionsToDelete.length > 0) {
      ...
      logToConsole("success", `Deleted ${versionsToDelete.length} old metadata versions`);
    }
...
async function deleteChatFromCloud(chatId) {
  ...
  try {
    ...
    try {
      ...
      logToConsole("success", `Successfully deleted from S3: chats/${chatId}.json`);
    } catch (error) {
      if (error.code !== "NoSuchKey") {
        throw error;
      }
...
async function downloadSettingsFromCloud() {
  ...
  try {
    ...
    try {
      ...
      const fileTimestamp = data.LastModified ? new Date(data.LastModified).toISOString() : "unknown";
      logToConsole("info", `Downloaded settings file from cloud. Last modified: ${fileTimestamp}`);
      const encryptedContent = new Uint8Array(data.Body);
      const settingsData = await decryptData(encryptedContent); // This now returns parsed JSON
      
      if (settingsData && typeof settingsData === "object" && !Array.isArray(settingsData)) {
        const settingsCount = Object.keys(settingsData).length;
        logToConsole("debug", `Processing ${settingsCount} parsed settings from cloud`, {
            keys: Object.keys(settingsData).slice(0, 10).join(", ") + (settingsCount > 10 ? "..." : ""),
        });
        const settingPromises = Object.entries(settingsData).map(async ([key, settingValue]) => {
            if (shouldExcludeSetting(key) || !settingValue || settingValue.data === undefined) {
                if (!shouldExcludeSetting(key)) {
                    logToConsole("warn", `Invalid setting format or excluded key skipped: ${key}`, settingValue);
                }
                return;
            }
            ...
            if (!currentLocalMeta || currentLocalMeta.hash !== newHash || currentLocalMeta.syncedAt < cloudTimestamp) {
                logToConsole("info", `Applying downloaded setting: ${key} from ${settingValue.source}. Reason: ` +
                    (!currentLocalMeta ? "New setting" : currentLocalMeta.hash !== newHash ? "Hash mismatch" : "Cloud newer"));
                settingsApplied = true;
                try {
                    let valueToStore = settingValue.data;
                    if (settingValue.source === "indexeddb") {
                        if (typeof valueToStore === "string" && (valueToStore.startsWith("{") || valueToStore.startsWith("["))) {
                            try { valueToStore = JSON.parse(valueToStore); } catch (e) { logToConsole("warning", `Could not parse IndexedDB value for ${key} as JSON, storing as string.`); }
                        }
                        await setIndexedDBKey(key, valueToStore);
                    } else {
                        localStorage.setItem(key, valueToStore);
                    }
                    if (!localMetadata.settings.items[key]) localMetadata.settings.items[key] = {};
                    localMetadata.settings.items[key].hash = newHash;
                    localMetadata.settings.items[key].lastModified = itemTimestamp;
                    localMetadata.settings.items[key].syncedAt = itemTimestamp;
                    localMetadata.settings.items[key].source = settingValue.source;
                    metadataNeedsSaving = true;
                } catch (error) {
                    logToConsole("error", `Error applying downloaded setting ${key}:`, error);
                }
            } else {
                if (currentLocalMeta.syncedAt < cloudTimestamp) { currentLocalMeta.syncedAt = cloudTimestamp; metadataNeedsSaving = true; }
                if (currentLocalMeta.lastModified < cloudTimestamp) { currentLocalMeta.lastModified = cloudTimestamp; metadataNeedsSaving = true; }
                logToConsole("debug", `Skipping setting ${key}: Local hash matches cloud and timestamp is not older.`);
            }
        });
        await Promise.all(settingPromises);
        if (settingsApplied) { logToConsole("info", "Finished applying downloaded settings."); } else { logToConsole("info", "No applicable setting changes found in downloaded settings."); }
        if (metadataNeedsSaving) { await saveLocalMetadata(); logToConsole("debug", "Saved local metadata changes after applying settings."); }
      } else if (settingsData) {
        logToConsole("error", "Parsed settings data was not a valid object. Skipping settings update.", { parsedType: typeof settingsData });
      }
...
async function uploadSettingsToCloud(syncTimestamp = null) {
  try {
    ...
    logToConsole("debug", `Beginning settings upload, timestamp: ${new Date(now).toISOString()}`);
    for (const key of Object.keys(localStorage)) {
      if (!shouldExcludeSetting(key)) {
        const value = localStorage.getItem(key);
        if (value !== null) {
    ...
    try {
      ...
      const keys = await new Promise((resolve, reject) => {
        const request = store.getAllKeys();
        request.onerror = (e) => reject(new Error(`Failed to get all keys: ${e.target.error}`));
      ...
      const readPromises = relevantKeys.map(
        (key) =>
          new Promise((resolve, reject) => {
            const request = store.get(key);
            request.onerror = (e) => reject(new Error(`Failed to read key ${key}: ${e.target.error}`));
      ...
      await new Promise((resolve, reject) => {
        ...
        transaction.onerror = (e) => reject(new Error(`Read transaction failed: ${e.target.error}`));
      });
      ...
      logToConsole("debug", `Successfully read ${Object.keys(settingsFromDB).length} settings from IndexedDB in single transaction.`);
    } catch (error) {
    ...
    logToConsole("success", "Uploaded settings to cloud", {
      ...
      settingKeys: Object.keys(settingsData).slice(0, 10).join(", ") + (Object.keys(settingsData).length > 10 ? "..." : ""),
    ...
    logToConsole("success", "✅ Cloud metadata updated after settings sync", {
      timestamp: new Date(now).toISOString(),
      settingsLastModified: new Date(cloudMetadata.settings.lastModified).toISOString(),
  ...
  try {
    ...
    try {
      ...
      if (error.code === "NoSuchKey" || error.name === "NoSuchKey") {
        logToConsole("info", "No cloud metadata found, creating initial metadata");
        ...
        try {
          ...
          logToConsole("error", "Failed to create initial metadata", uploadError);
          throw uploadError;
        }
      }
      throw error;
    ...
    if (
      ...
      logToConsole("error", "AWS credentials error - please check your configuration");
...
async function downloadChatFromCloud(chatId) {
  ...
  try {
    ...
    try {
      const data = await s3.getObject(params).promise();
      const encryptedContent = new Uint8Array(data.Body);
      const chatData = await decryptData(encryptedContent); // Returns parsed JSON
      ...
      if (!chatData.id) {
        ...
        logToConsole("warning", `Chat ID mismatch: ${chatData.id} !== ${chatId}, using key as ID`);
        chatData.id = chatId;
      }
      ...
      if (error.code === "NoSuchKey") {
        ...
        if (
          ...
          logToConsole("info", `Created tombstone for missing chat ${chatId} in cloud metadata`);
        }
        return null;
      }
      throw error;
...
async function uploadChatToCloud(
  ...
  try {
    ...
    if (chatData.id !== chatId) {
      logToConsole("warning", `Chat ID mismatch: ${chatData.id} !== ${chatId}, fixing before upload`);
      chatData.id = chatId;
    }
    const newHash = await generateHash(chatData, "chat");
    if (
...
async function checkSyncStatus() {
  if (!isAwsConfigured()) {
    logToConsole("debug", "checkSyncStatus returning: disabled (AWS not configured)");
    return "disabled";
  }
  await loadLocalMetadata();
  try {
...
function updateSyncStatusDot(status) {
  ...
  switch (status) {
    ...
    case "out-of-sync":
      ...
      break;
    default:
      logToConsole("debug", `updateSyncStatusDot hit default case for status: ${status}`);
      dot.style.backgroundColor = "#6b7280";
  }
}
    ...
    for (const key of indexedDBKeys) {
      if (localStorageKeys.includes(key)) {
        try {
          ...
          logToConsole("cleanup", `Removed duplicate key from IndexedDB: ${key}`);
        } catch (error) {
          errorKeys.push(key);
          logToConsole("error", `Failed to remove duplicate key ${key} from IndexedDB:`, error);
        }
      }
    }
    if (cleanedKeys.length > 0) {
      logToConsole("success", `IndexedDB cleanup completed. Removed ${cleanedKeys.length} duplicate keys:`, cleanedKeys);
      if (localMetadata.settings && localMetadata.settings.items) {
        for (const key of cleanedKeys) {
          if (localMetadata.settings.items[key]) {
            ...
            logToConsole("cleanup", `Removed ${key} from sync metadata tracking`);
          }
        }
        await saveLocalMetadata();
      }
      localStorage.removeItem("sync_duplicateDetected");
      logToConsole("success", "Duplicate detection flag cleared - cleanup complete");
    } else {
      logToConsole("info", "No duplicate keys found in IndexedDB");
      localStorage.removeItem("sync_duplicateDetected");
    }
    if (errorKeys.length > 0) {
      logToConsole("warning", `Failed to clean ${errorKeys.length} keys:`, errorKeys);
    }
  } catch (error) {
...
async function detectIndexedDBDuplicates() {
  try {
    ...
    const localStorageKeys = Object.keys(localStorage);
    const duplicates = indexedDBKeys.filter((key) => localStorageKeys.includes(key));
    if (duplicates.length > 0) {
      localStorage.setItem("sync_duplicateDetected", "true");
      logToConsole("info", `Detected ${duplicates.length} duplicate keys between IndexedDB and localStorage`, duplicates);
      return true;
    } else {
      localStorage.removeItem("sync_duplicateDetected");
      logToConsole("info", "No duplicates detected between IndexedDB and localStorage");
      return false;
    }
  } catch (error) {
